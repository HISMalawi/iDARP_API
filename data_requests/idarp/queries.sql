...
SELECT s.approval_procedure_id, s.stage_id, s.stage_order, t.stage_type, s.stage_activity, s.branch_level, s.role_id, ors.org_role_id, ors.role, s.pos_x, s.pos_y, s.icon
FROM v_active_org_roles ors INNER JOIN stages s ON ors.role_id = s.role_id INNER JOIN stage_types t ON s.stage_type_id = t.stage_type_id
